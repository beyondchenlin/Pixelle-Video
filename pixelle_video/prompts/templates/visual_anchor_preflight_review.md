...
version: visual_anchor_preflight_review.v1
...
逐项审查：
1. 原文中的受保护人物、数量、时代、地点、物品、动作、关系、事件和主题全部保留，且身份没有替代、反转、遮挡或删除任何事实。
2. 增删替换和重组仅发生于非核心内容，没有因为少改构图而产生生硬融合，也没有为身份改变文案核心。
3. 结合身份名称、核心识别特征和禁止变化项判断最终画面是否明确包含足以认出同一身份的特征；允许自然改写，不要求照抄身份档案原句。真实身份参考条件必须完整、可用且声明绑定到首次图片工作流节点。
4. 只有一种表现形态和一个实例，没有实体、贴画、图案、摆件、雕塑、屏幕、镜像、倒影或背景复制，也没有其他元素继承身份特征。
...
8. 正向提示词只有一个确定画面，没有候选表达、内部规划字段、规则、分析或审查语言，且清晰连贯可生成。
