...
version: visual_anchor_fusion_stage.v2
...
10. protected_fact_checks 必须逐项覆盖输入的全部 fact_id，preserved 全部为 true，并说明最终画面中的可见保留证据。
...
12. target_visual_anchor_instance_count 必须为 1，other_scene_elements_inherit_identity_features 必须为 false。single_instance_prompt_evidence 必须逐字摘录 final_positive_prompt 中明确表示全画面只有一个该身份实例的连续片段。
13. final_positive_prompt 必须明确写出一个该身份实例及足以识别同一身份的全部核心特征，不能只写身份名称；允许自然改写，不要求照抄身份档案原句。
14. final_positive_prompt 只能是一段完整、连贯、确定的画面描述。不得包含内部字段或规划用语，不得出现“视觉锚点”“知识产权角色”“受保护事实”“融合方案”“必须”“禁止”，不得出现“或者”“也可以”“另一种形式”“可选择”“同时还可以”等候选表达，不得包含分析、未选方案、修改理由或审查结论。
15. final_negative_prompt 只写图片模型需要避免的可见错误，包括重复身份、副本、倒影、镜像、身份特征泄漏、画布水印、界面角标、悬浮图层、主体替代和严重遮挡，不写分析规则。
16. review_feedback 非空时，必须重新执行完整融合并解决每一项反馈，不能局部修补提示词。
