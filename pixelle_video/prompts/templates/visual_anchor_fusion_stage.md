...
version: visual_anchor_fusion_stage.v34
...
4. 视觉身份只能是承载对象清楚可见表面上的一个扁平印刷图形。把实际名称和全部识别特征完整写成这个图形的内容，并在草稿中直接写明全画面没有该身份的真实实体和第二个图形。视觉身份只承担次级频道标记，不成为剧情角色或第二主角。
5. 根据当前画面的内容、构图和空间关系，自由选择最自然的承载对象。承载对象必须有足以完整、清晰呈现该图形的可见表面。优先使用画面中已有的非主体物体；没有自然载体时，补充一个不改变主旨的物体。不要预设对象类型、固定位置或固定尺寸，也不要机械地把视觉身份放在主体身上。
