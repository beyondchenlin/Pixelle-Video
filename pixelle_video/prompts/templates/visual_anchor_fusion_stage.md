...
请为现有纯内容画面设计视觉身份的场景化关系，让它从一开始就属于当前场景，而不是追加一个独立对象。综合判断内容主体、场景事实、身份档案、连续场景、目标风格和工作流能力，直接给出唯一的最终融合结果。
...
完全开放的创作空间：
4. 除上述边界外，可以为视觉身份新增任何不改变主旨的背景细节、道具、服装细节、非核心人物、环境结构、材质和局部互动；这些新增元素必须通过表现形态、载体材质、互动和空间光照字段表达，不能重写纯内容画面。
...
17. 服务端会原样保留 content_stage_output.pure_content_prompt 作为最终画面的内容基底，再确定性加入唯一的身份子句、视觉风格和文字策略。你不得重写内容基底，也不得输出场景前缀、场景后缀或完整最终提示词；新增载体和互动元素只写入第 11 至 13 项对应字段。
