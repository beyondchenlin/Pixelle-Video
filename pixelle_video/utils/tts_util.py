...
async def edge_tts(
    ...
    async with request_semaphore:
        ...
        for attempt in range(retry_count + 1):  # +1 because first attempt is not a retry
            ...
            try:
            ...
            except _RETRYABLE_EDGE_TTS_ERRORS as e:
                ...
                if error_code == 401 or "401" in error_msg:
                    logger.warning(
                        f"Edge TTS 401 Authentication Error (attempt {attempt + 1}/{retry_count + 1})"
                    )
                    ...
                    logger.warning(
                        f"Edge TTS error (attempt {attempt + 1}/{retry_count + 1}): {error_code} - {e}"
                    )
                ...
                last_error = e
                logger.warning(f"Edge TTS NoAudioReceived (attempt {attempt + 1}/{retry_count + 1})")
            ...
            # Local persistence failures should surface immediately instead of
            # triggering another remote TTS request.
            if output_path:
    ...
    async with request_semaphore:
        ...
        for attempt in range(retry_count + 1):
            ...
            try:
                ...
                if error_code == 401 or "401" in error_msg:
                    logger.warning(
                        f"Edge TTS 401 Authentication Error (list_voices attempt {attempt + 1}/{retry_count + 1})"
                    )
                    logger.debug(f"Error details: {error_msg}")
                    logger.debug("This is usually caused by rate limiting. Will retry with exponential backoff...")
                else:
                    logger.warning(
                        f"List voices error (attempt {attempt + 1}/{retry_count + 1}): {error_code} - {e}"
                    )
