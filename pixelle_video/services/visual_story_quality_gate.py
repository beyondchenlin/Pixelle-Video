...
class VisualStoryQualityGate:
    ...
    def validate_context(self, context: Mapping[str, Any]) -> list[VisualStoryQualityFinding]:
        ...
        if len(visual) != len(fusion):
            findings.append(_error("context_frame_fusion_count_mismatch", "visual story context frame counts do not match"))
        by_id = {str(item.get("frame_id") or ""): item for item in visual if isinstance(item, Mapping)}
        for item in fusion:
    ...
    def assert_context_valid(self, context: Mapping[str, Any]) -> None:
        ...
        errors = [finding for finding in findings if finding.severity == "error"]
        if errors:
            raise ValueError("visual story context quality gate failed: " + "; ".join(error.message for error in errors))


def _validate_mandatory_ip_duty(frame_id: str, fusion_plan: Any, findings: list[VisualStoryQualityFinding]) -> None:
    if fusion_plan.ip_duty_preset in {IPDutyPreset.AUTO, IPDutyPreset.NONE, IPDutyPreset.BACKGROUND_SIGNATURE}:
        findings.append(_error("missing_content_bound_ip_duty_preset", f"{frame_id}: active IP frames require a concrete non-background content duty"))
...
def _validate_content_bound_ip(frame_id: str, frame_plan: Any, fusion_plan: Any, findings: list[VisualStoryQualityFinding]) -> None:
    payload = fusion_plan.to_dict() if hasattr(fusion_plan, "to_dict") else dict(fusion_plan or {})
    visual_payload = frame_plan.to_dict() if hasattr(frame_plan, "to_dict") else dict(frame_plan or {})
    _validate_content_bound_payload(frame_id, visual_payload, payload, findings)


def _validate_content_bound_payload(frame_id: str, frame_plan: Mapping[str, Any], fusion_plan: Mapping[str, Any], findings: list[VisualStoryQualityFinding]) -> None:
    if fusion_plan.get("rewrite_required"):
        findings.append(_error("rewrite_required_not_consumed", f"{frame_id}: rewrite_required must trigger repair before prompt composition"))
    if str(fusion_plan.get("content_relation_type") or "") != "content_bound":
        findings.append(_error("missing_content_bound_relation", f"{frame_id}: IP fusion must declare content_relation_type=content_bound"))
    if not isinstance(fusion_plan.get("content_bound_ip_presence_plan"), Mapping):
        findings.append(_error("missing_content_bound_presence_plan", f"{frame_id}: IP fusion must carry content_bound_ip_presence_plan"))
    for field in ("cognitive_anchor", "physical_metaphor", "scene_arena", "ip_action_affordance"):
        if not str(frame_plan.get(field) or fusion_plan.get(field) or "").strip():
            findings.append(_error(f"missing_{field}", f"{frame_id}: content-bound frame plan requires {field}"))
    text = _mapping_text(fusion_plan, (
        "placement_logic",
        "action_or_function",
    ...
    if contains_decorative_ip_language(text):
        findings.append(_error("decorative_ip_carrier", f"{frame_id}: IP uses decorative carrier language instead of content-bound participation"))
    if contains_weak_ip_action_language(text):
        findings.append(_error("weak_ip_action", f"{frame_id}: IP action is too weak or decorative"))
    try:
        score = float(fusion_plan.get("decorative_risk_score") or 0.0)
    except (TypeError, ValueError):
        score = 1.0
    if score > 0.3:
        findings.append(_error("decorative_risk_score_too_high", f"{frame_id}: decorative_risk_score must be <= 0.3"))
    removal_test = str(fusion_plan.get("semantic_removal_test") or "")
    if not removal_test or not any(token in removal_test.lower() for token in ("weak", "weaken", "削弱", "失去", "loses")):
        findings.append(_error("missing_semantic_removal_test", f"{frame_id}: IP must pass semantic removal test"))
