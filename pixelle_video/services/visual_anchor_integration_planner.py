...
from pixelle_video.models.visual_role_strategy import VisualRoleStrategyControls, build_visual_identity_kernel
...
class VisualAnchorIntegrationPlanner:
    """Mandatory visual-role integration planner.
    ...
    async def plan_batch(
        ...
        *,
        ...
        if anchor_profile is None:
            raise ValueError("mandatory visual role integration requires anchor_profile")
        if self.llm_service is None:
            raise ValueError("mandatory visual role integration requires llm_service")

        policy = self.policy or load_visual_signature_policy()
        role_strategy = self.visual_role_strategy or VisualRoleStrategyControls()
        ...
        for attempt in range(max(1, self.max_repair_attempts + 1)):
            rendered_prompt = render_visual_anchor_integration_prompt(
                ...
                cadence_plan_json=[],
                visual_role_strategy_json=role_strategy.to_dict(),
            ...
            logger.warning(
                "mandatory visual role integration rejected attempt {}: {}",
                attempt + 1,
                "; ".join(errors[:8]),
            )

        raise ValueError(
            "mandatory visual role integration failed after repair attempts: "
...
def _anchor_profile_payload(anchor_profile: IPProfile, *, policy: VisualSignaturePolicy, identity_kernel: Sequence[str]) -> dict[str, Any]:
    return {
        ...
        "guidance": [
            "This is mandatory visual-role integration.",
...
def _placement_plans_from_payload(
    ...
    *,
    ...
    role_strategy: VisualRoleStrategyControls,
...
def _candidate_to_plan(
    ...
    *,
    ...
    return VisualAnchorPlacementPlan(
        ...
        metadata={
            "source": "llm_mandatory_visual_role_integration",
            "projection": "llm_integrated_prompt",
            "mandatory_integration": True,
            "visual_role_strategy": role_strategy.to_dict(),
