...
class VideoService:
    ...
    def merge_audio_video(
        ...
        if auto_adjust_duration:
            ...
            if diff < 0:
                ...
                logger.info(
                    f"⚠️ Video longer than audio by {diff:.2f}s (tolerance: {duration_tolerance}s)"
                )
                ...
                logger.info(
                    f"✅ Duration acceptable: video={video_duration:.2f}s, audio={audio_duration:.2f}s (diff={diff:.2f}s)"
                )
        ...
        if audio_duration > video_duration:
            ...
            logger.info(
                f"Audio is longer, padding video by {pad_duration:.2f}s using '{pad_strategy}' strategy"
            )
        ...
        try:
            ...
            if scale_mode == "contain":
                ...
                scaled_video = input_video.filter(
                    "scale", overlay_width, overlay_height, force_original_aspect_ratio="decrease"
                ).filter(
                    "pad", overlay_width, overlay_height, "(ow-iw)/2", "(oh-ih)/2", color="black"
                )
            elif scale_mode == "cover":
                # Scale to cover (crop if aspect ratio differs)
                scaled_video = input_video.filter(
                    "scale", overlay_width, overlay_height, force_original_aspect_ratio="increase"
    ...
    def add_bgm(
        ...
        try:
            ...
            if not math.isfinite(video_duration) or video_duration <= 0:
                raise RuntimeError("Failed to add BGM: video duration is invalid")
            bgm_duration = video_duration if loop else self._get_audio_duration(bgm)
            if not math.isfinite(bgm_duration) or bgm_duration <= 0:
                raise RuntimeError("Failed to add BGM: audio duration is invalid")
            audible_bgm_duration = min(video_duration, bgm_duration)
            ...
            bgm_audio = (
                bgm_input.audio.filter("volume", bgm_volume)
                .filter("atrim", duration=_ffmpeg_duration(audible_bgm_duration))
                .filter("asetpts", "N/SR/TB")
            )
            ...
            if fade_in > 0:
                bgm_audio = bgm_audio.filter(
                    "afade",
                    type="in",
                    duration=_ffmpeg_duration(min(fade_in, audible_bgm_duration)),
                )
            if fade_out > 0:
                fade_duration = min(fade_out, audible_bgm_duration)
                bgm_audio = bgm_audio.filter(
                    "afade",
                    type="out",
                    start_time=_ffmpeg_duration(max(audible_bgm_duration - fade_duration, 0.0)),
                    duration=_ffmpeg_duration(fade_duration),
                )

            # Keep the audio timeline aligned with the complete video even when
            # either input audio stream ends early.
            bgm_audio = bgm_audio.filter("apad").filter(
                "atrim", duration=_ffmpeg_duration(video_duration)
            )
            if self.has_audio_stream(video):
                original_audio = (
                    input_video.audio.filter("apad")
                    .filter("atrim", duration=_ffmpeg_duration(video_duration))
                    .filter("asetpts", "N/SR/TB")
                )
    ...
    def _trim_video_to_duration(self, video: str, target_duration: float) -> str:
        ...
        try:
            ...
            (
                ffmpeg.input(video, t=_ffmpeg_duration(target_duration))
