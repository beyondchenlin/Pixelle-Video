...
class VisualStyleContractResolver:
    def resolve(
        ...
        *,
        ...
        selected_style_item: Mapping[str, Any] | None = None,
        fallback_to_default_world: bool = True,
    ) -> VisualStyleLayerContract:
        contracts: list[VisualStyleLayerContract] = []
        item_contract = _contract_from_selected_item(selected_style_item)
...
def _contract_from_selected_item(selected_style_item: Mapping[str, Any] | None) -> VisualStyleLayerContract | None:
    if not isinstance(selected_style_item, Mapping):
        return None
    raw_contract = selected_style_item.get("visual_style_contract")
    if isinstance(raw_contract, Mapping):
        return VisualStyleLayerContract.from_dict(raw_contract)
    if selected_style_item.get("style_contract_kind") == "visual_style_contract":
        layers = selected_style_item.get("visual_style_layers")
        if layers:
            return VisualStyleLayerContract.from_dict(
                {
                    "layers": layers,
                    "integration_rules": selected_style_item.get("integration_rules") or (),
                    "negative_rules": selected_style_item.get("negative_rules") or (),
                }
            )
    return None
