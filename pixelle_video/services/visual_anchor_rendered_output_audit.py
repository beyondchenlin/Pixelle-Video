...
class VisualAnchorRenderedOutputAudit:
    ...
    async def evaluate(
        ...
        *,
        ...
        if request.identity_conditioning_mode == "text_profile":
            return await self._evaluate_text_profile(
    ...
    async def _evaluate_text_profile(
        self,
        *,
        image_path: str | Path,
        ...
        def check(name: str, condition: bool, failure_code: str | None = None) -> None:
            checks[name] = condition
            if not condition:
                failures.append(failure_code or name)

        check("generated_image_exists", image is not None, "generated_image_missing")
        check(
            "first_request_binding_artifact_exists",
            binding_path.is_file(),
            "first_request_binding_missing",
        ...
        comparisons = (
            ...
            (
                ...
                and binding.get("reference_condition") is None,
            ),
            (
                "target_visual_style_preserved",
                binding.get("target_visual_style")
            ...
            (
                "actual_reference_input_count_zero",
                actual_execution.get("reference_conditioning_input_count") == 0,
            ),
            (
                "downloaded_image_matches_first_comfyui_output",
                actual_execution.get("generated_output_sha256") == image_sha256,
            ...
            (
                "actual_execution_config_version_preserved",
                actual_execution.get("execution_config_sha256")
                == _canonical_json_sha256(execution_config),
            ),
        )
        for name, condition in comparisons:
