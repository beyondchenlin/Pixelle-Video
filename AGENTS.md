...
- 视觉锚点的内容与融合阶段，每个分镜各只允许一次大模型调用；所有创作要求只允许写在调用前的提示词中。两个阶段都使用原始文本响应，不要求结构化创作字段。大模型输出后，本地不得解析、判断、验证、修复或改写创作内容；融合阶段返回的原始文本必须直接作为图片正向提示词。效果问题必须通过调整提示词解决。禁止自动重试、修复性重试、携带反馈再次调用或增加大模型审查阶段。
