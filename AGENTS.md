...
- `commit` 的含义固定为：提交到本地仓库。
- `push` 的含义固定为：推送到 GitHub 远端。
- 在与用户沟通 Git 操作时，必须明确区分 `commit` 与 `push`，禁止混用、偷换或省略导致歧义。
- 除非用户明确要求，否则不得把“本地提交”表述为“已经提交到 GitHub”。
- 禁止在本仓库任务中使用 `worktree`、`git worktree` 或基于 worktree 的隔离工作目录方案。
- 如需隔离改动范围，必须在当前工作区内通过原子化暂存、原子化提交或显式分支策略处理，不能引入 worktree。
