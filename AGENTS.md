...
- 视觉锚点固定使用内容生成、融合草稿、最终审查重写三个阶段，每个分镜的每个阶段各只允许一次大模型调用；所有创作与审查要求只允许写在对应调用前的提示词中。三个阶段都使用原始文本响应，不要求结构化创作字段。大模型输出后，本地不得解析、判断、验证、修复或改写创作内容；最终审查重写阶段返回的原始文本必须直接作为图片正向提示词。禁止自动重试、修复性重试、失败后追加调用、携带反馈再次调用或生成图片后的自动审查与重生成。
