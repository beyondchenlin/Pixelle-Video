...
def test_production_runtime_profile_treats_whitespace_backend_env_as_missing(monkeypatch):
    _clear_runtime_env(monkeypatch)
    monkeypatch.setenv("PIXELLE_RUNTIME_PROFILE", "production")
    monkeypatch.setenv("PIXELLE_TASK_BACKEND", " ")
    ...
    monkeypatch.setenv("PIXELLE_ARTIFACT_BACKEND", "\t")
    ...
    message = str(exc_info.value)
    assert "PIXELLE_TASK_BACKEND=postgres" in message
    assert "PIXELLE_ARTIFACT_BACKEND=s3" in message
