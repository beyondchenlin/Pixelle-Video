...
def test_fusion_template_keeps_facts_fixed_and_restores_whole_scene_recomposition():
    ...
    for required_rule in (
        ...
        "整幅画只出现一个可识别的视觉身份实例",
        "不得概括、省略或改写",
        "视觉身份只能是承载对象清楚可见表面上的一个扁平印刷图形",
        "把实际名称和全部识别特征完整写成这个图形的内容",
        "全画面没有该身份的真实实体和第二个图形",
        "视觉身份只承担次级频道标记",
        "自由选择最自然的承载对象",
        "优先使用画面中已有的非主体物体",
        "承载对象必须有足以完整、清晰呈现该图形的可见表面",
...
def test_finalization_template_unifies_scene_and_signature_styles():
    ...
    for required_rule in (
        ...
        "不得概括、省略或改写",
        "视觉身份只能是承载对象清楚可见表面上的一个扁平印刷图形",
        "把 identity_profile.display_name 和 identity_profile.core_identity_traits 的实际内容逐项完整写成这个图形的内容",
        "全画面没有该身份的真实实体和第二个图形",
        "自由选择最自然的承载对象",
        "优先使用画面中已有的非主体物体",
        "承载对象必须有足以完整、清晰呈现该图形的可见表面",
