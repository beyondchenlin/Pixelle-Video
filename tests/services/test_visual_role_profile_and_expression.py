...
def test_visual_role_profile_builder_supports_human_host():
    profile = VisualRoleProfileBuilder().build(_profile("人类主持人", ip_type="human"))
    assert "人类主持人" in profile.identity_kernel
    assert "固定主持人" in profile.primary_role_affordances


def test_visual_role_profile_builder_supports_red_beaked_sparrow():
    profile = VisualRoleProfileBuilder().build(_profile("红嘴麻雀", ip_type="animal"))
    assert "红嘴麻雀" in profile.identity_kernel
    assert "故事行动者" in profile.primary_role_affordances


def test_visual_role_profile_builder_supports_airplane():
    profile = VisualRoleProfileBuilder().build(_profile("飞机", ip_type="vehicle"))
    assert "飞机" in profile.identity_kernel
    assert "运动主体" in profile.primary_role_affordances


def test_visual_role_profile_builder_supports_stone_object():
    profile = VisualRoleProfileBuilder().build(_profile("石头", ip_type="object"))
