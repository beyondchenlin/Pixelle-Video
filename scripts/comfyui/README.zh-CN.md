...
后端配置中的 `vram_mode`（显存模式）默认是 `normal`（正常卸载）。在已验证模型与工作流能够装入显卡时，可为单个图片后端设置 `high`（显卡驻留），减少模型回迁系统内存造成的峰值。它不修改模型、图片尺寸、采样参数或三阶段提示词；语音后端可继续使用默认值。

```yaml
comfyui:
  backends:
    image:
      vram_mode: high  # 已测量显存足够时，启用显卡驻留
```

启动、检查和停止脚本都接受 `-VramMode normal`（正常卸载）或 `-VramMode high`（显卡驻留），并把它纳入进程身份检查；不同模式不能被误认为同一托管实例。切换前先让当前批次完成并正常停止后端。显存不足的机器保留默认模式；不在失败后自动切换或重生成。批次结束仍通过完整进程退出释放资源。

本机量化文字模型与图片模型加载、复用、卸载实验：正常模式私有内存峰值约 29.1 吉字节，显卡驻留约 17.5 吉字节，单独关闭动态显存管理约 29.1 吉字节。上述只是两模型实验，不是所有工作流的容量承诺；完整成片实测见[实施记录](../../docs/superpowers/plans/2026-09-05-视频生产改进实施记录.md)。
