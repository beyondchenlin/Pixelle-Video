...
- 用途：Pixelle API 可传参执行的 OmniVoice bf16 语音克隆定长 TTS 工作流。
...
- `OmniVoiceVoiceCloneTTS`：语音克隆 TTS 主节点，使用 `OmniVoice-bf16`。
- `SaveAudio`：保存 FLAC 音频。

`PixelleDurationInput` 专用于 `duration` 秒数参数，默认值为 `8.0`。不要复用 `PixelleFloatInput`，它保留给 Edge TTS 的 `speed` 参数，避免两个 API 参数在语义和校验范围上混淆。
...
- `PixelleDurationInput` 来自 `ComfyUI-Pixelle-TTS`，用于暴露 `duration` 秒数；不要使用 `PixelleFloatInput`，后者保留给 Edge TTS speed 参数。
- `duration` 适合短文本定长控制，不建议用于长文本整体控时；长文本请使用 `tts_omnivoice_longform_bf16.json`。
