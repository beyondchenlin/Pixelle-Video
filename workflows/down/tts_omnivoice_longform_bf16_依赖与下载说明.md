...
- 用途：Pixelle 可传参执行的 OmniVoice bf16 长文本旁白 TTS 工作流。
...
- 如果参考文本为空，OmniVoice 节点可能回退到 Whisper 自动转写；保存音色场景建议传入 `reference_audio_text`。
- 该工作流用于长文本旁白，`duration` 固定为 `0`，由模型内部 `words_per_chunk` 处理长文本分块。
