...
# 本地量化模型卸载兼容性补充（2026-09-05）

若生图后端在模型加载清理时出现 Windows（视窗系统）内存访问崩溃，堆栈经过量化插件的 `unpatch_model`（解除模型补丁方法）与基础模型的设备迁移，参考[上游第 445 号修复](https://github.com/city96/ComfyUI-GGUF/pull/445/files)，对应提交 `b543b78`。该上游改动仍为待合并状态，不能将其描述为官方已发布版本。

项目提供定向维护命令：

```powershell
# 将占位路径替换为实际安装的量化插件目录；先停止使用该插件的生图任务。
python scripts/comfyui/repair_gguf_unpatch.py "<量化插件目录>"
```

脚本只接受已核对的旧方法，修改前自动保存带内容摘要的备份，保留其余本地定制；来源不匹配时拒绝修改。它保留基础权重恢复、挂钩清理和内存计数，仅跳过对同设备权重的重复迁移。恢复时停止生图进程，将备份复制回原文件；不要在运行中的模型上切换代码。

本机已执行该维护命令，并使用原有默认量化主模型完成真实显卡加载、卸载和进程正常退出验证；修复脚本三项测试通过。此项只证明模型加载与卸载检查通过，完整生成结果另见[视频生产实施记录](../../docs/superpowers/plans/2026-09-05-视频生产改进实施记录.md)。没有新增或下载模型资源。
