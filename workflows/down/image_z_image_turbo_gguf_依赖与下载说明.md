...
当前默认工作流 `image_z_image_turbo_gguf.json` 使用 `Q4_K_M`，比原来的 `Q8_0` 显存占用更低，适合作为项目默认值。高显存机器如果要使用 Q8，可以选择单独的 `image_z_image_turbo_gguf_q8.json` 工作流；这样默认低显存链路和高精度链路不会互相覆盖。

如果需要在质量与显存之间继续取舍，可以基于独立工作流里的文件名替换成同一仓库里的其他量化版本，例如：
