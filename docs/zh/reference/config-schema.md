...
- `gguf_cleanup_strategy`: ComfyUI-GGUF 工作流的清理边界。当前正常阶段固定使用 `"extension_release"`：先释放 ComfyUI 标准模型，再调用 `/pixelle/gguf/free` 释放 GGUF 扩展缓存。旧配置中的 `"process_restart"` 会迁移为 `"extension_release"`；进程重启只用于后端崩溃或连接丢失后的恢复。
