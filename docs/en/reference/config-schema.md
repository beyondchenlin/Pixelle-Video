...
- `model_cleanup_mode`: Model memory cleanup scope used at Pixelle-owned local workflow stage boundaries and explicit recovery paths. `disabled` leaves models loaded after the stage. `comfyui` calls ComfyUI `/free` for standard stages, but IndexTTS2 TTS still requires the Pixelle-managed plugin cleanup endpoint. `comfyui_and_extensions` keeps the same required IndexTTS2 cleanup and is the recommended default for local dedicated ComfyUI.
- `comfyui_api_key`: ComfyUI API key (optional, for [Comfy Platform](https://platform.comfy.org/profile/api-keys))

Pixelle assumes the configured self-hosted ComfyUI instance is dedicated to Pixelle. It does not call `/free` before each image in a local image batch; the batch keeps GGUF and related models hot so every frame does not pay the unload/reload cost. When the image stage finishes, Pixelle releases ComfyUI-managed models before moving to the next stage. When an IndexTTS2 TTS stage finishes, Pixelle releases ComfyUI-managed models and then the patched `/pixelle/indextts2/free` plugin cache. If local IndexTTS2 runs while model cleanup is not `disabled`, Pixelle preflights the side-effect-free `/pixelle/indextts2/health` endpoint before executing the workflow so a missing plugin patch fails early without unloading hot models. If stage-boundary release is not confirmed, Pixelle fails the task instead of entering the next model stage. OOM recovery still triggers an explicit high-intensity cleanup before one retry.
