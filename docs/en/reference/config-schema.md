...
- `backend_management_mode`: Pixelle-managed ComfyUI backend supervision policy. `"auto"` manages only the local Pixelle backend at `127.0.0.1:8000`, `"required"` fails if managed restart is unavailable, and `"disabled"` never starts or stops ComfyUI.
...
- `gguf_cleanup_strategy`: Cleanup boundary for ComfyUI-GGUF workflows. `"process_restart"` is the recommended default for the Pixelle-managed backend because native Torch/GGUF access violations can bypass Python cleanup; `"extension_release"` keeps the older in-process `/pixelle/gguf/free` path.
- `comfyui_api_key`: ComfyUI API key (optional, for [Comfy Platform](https://platform.comfy.org/profile/api-keys))

Pixelle assumes the configured self-hosted ComfyUI instance is dedicated to Pixelle. It does not call `/free` before each image in a local image batch; the batch keeps GGUF and related models hot so every frame does not pay the unload/reload cost. For the Pixelle-managed local backend on `127.0.0.1:8000`, GGUF stages use a process restart by default at the stage boundary, which clears native Torch/GGUF state that cannot be reliably recovered from inside the same Python process. When an IndexTTS2 TTS stage finishes, Pixelle releases ComfyUI-managed models and then the patched `/pixelle/indextts2/free` plugin cache. Local IndexTTS2 runs preflight the side-effect-free `/pixelle/indextts2/health` endpoint before executing the workflow so a missing plugin patch fails early without unloading hot models. If stage-boundary release or managed restart is not confirmed, Pixelle fails the task instead of entering the next model stage. OOM recovery also releases IndexTTS2 plugin caches before retrying an IndexTTS2 workflow. If a self-hosted workflow loses its HTTP connection because the managed backend crashed, Pixelle restarts the managed backend and retries the workflow once.
