...
Pixelle does not call `/free` before each image in a local image batch. The batch keeps GGUF and related models hot so every frame does not pay the unload/reload cost. When that local workflow batch finishes, `model_cleanup_mode` controls whether Pixelle releases ComfyUI model memory and Pixelle-managed extension caches. If a local IndexTTS2 workflow is used with `comfyui_and_extensions`, Pixelle preflights the side-effect-free `/pixelle/indextts2/health` endpoint before executing the workflow so a missing plugin patch fails early without unloading hot models. OOM recovery still triggers an explicit high-intensity cleanup before one retry.
