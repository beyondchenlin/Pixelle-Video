...
- `gguf_cleanup_strategy`: Cleanup boundary for ComfyUI-GGUF workflows. Normal stages now use `"extension_release"`: ComfyUI `/free` plus `/pixelle/gguf/free` for GGUF extension caches. Legacy `"process_restart"` config values are migrated to `"extension_release"`; process restart is reserved for backend crash or connection-loss recovery.
