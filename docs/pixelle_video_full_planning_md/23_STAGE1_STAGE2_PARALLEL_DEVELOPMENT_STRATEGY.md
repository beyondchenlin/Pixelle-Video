...
并行开发不是把三个阶段都接进主生成链路，而是让不同小组先实现各自独立的领域合同、服务接口和测试。只有当上游合同稳定后，才允许下游集成。
...
| --- | --- | --- | --- |
| Stage 1A Text / Prompt / Trace | `LLMInteractionTrace`、`PromptPlan`、`ImagePromptDraft`、Trace Store、Trace API | 无，当前最高优先级 | `pixelle_video/models/prompt_plan.py`、`pixelle_video/models/llm_interaction_trace.py`、`pixelle_video/services/llm_trace.py`、`pixelle_video/services/prompt_plan_service.py` |
| Stage 1B Workbench | `Artifact`、`ArtifactVersion`、`GenerationTrace`、工作台 lock/stale 模型、本地 JSON 服务 | 不能执行 PromptPlan 定义任务，必须消费 Stage 1A 合同 | `pixelle_video/models/artifact.py`、`pixelle_video/models/generation_event.py`、`pixelle_video/models/storyboard_workbench.py`、`pixelle_video/services/artifact_service.py` |
| Stage 2 IP / AssetBible | `AssetBible`、`IPProfile`、`CharacterProfile`、`SceneCast` 模型和校验器 | 主链路接入必须等 Stage 1A PromptPlan 字段稳定 | `pixelle_video/models/asset_bible.py`、`pixelle_video/models/scene_cast.py`、`pixelle_video/services/asset_bible_service.py`、`pixelle_video/services/scene_casting.py` |
...
```text
第一批并行：
  A1: Stage 1A LLMInteractionTrace 合同和本地存储
  A2: Stage 1A PromptPlan / ImagePromptDraft 合同
  B1: Stage 1B Artifact / ArtifactVersion 合同
  C1: Stage 2 AssetBible / IPProfile / CharacterProfile 合同

第二批并行：
  A3: Stage 1A LLMService 网关追踪
  A4: Stage 1A PromptPlanBuilder 接现有 StoryboardPlan / ImagePromptComposer
  B2: Stage 1B GenerationTrace / LocalJsonArtifactService
  C2: Stage 2 SceneCast 模型和校验器

第三批并行：
  A5: Stage 1A Trace API / Studio Trace Panel 数据合同
  B3: Stage 1B workbench select / regenerate / stale
