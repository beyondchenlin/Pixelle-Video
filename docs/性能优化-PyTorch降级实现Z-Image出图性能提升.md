# 性能优化：PyTorch 降级实现 Z-Image 出图性能提升

## 概述

2026/05/06 发现 ComfyUI Z-Image Turbo GGUF 工作流出图速度从正常的 2-3 秒/张退化到 7-11 秒/张。通过版本溯源和逐层排除，定位根因是 PyTorch 2.10.0 → 2.11.0 升级引入了多个已知性能退化，最终通过降级 PyTorch 恢复性能。
