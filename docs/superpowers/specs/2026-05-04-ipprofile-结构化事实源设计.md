...
3. 用嵌套、模型对齐的 AssetBible 保存合同替换当前缩水平铺合同；
4. 在 IP 设计工作台正式暴露结构化身份与约束字段；
5. 启用 IP 且身份锚点为空时直接阻断生成；
6. 不做旧数据迁移，不做旧字段兼容猜测，不做运行时补丁；
7. 在此基础上，再推进生成页世界观提示方案。
