...
素材尺寸默认继续独立于最终视频尺寸。图片素材默认使用横屏 `1280x720`，同时保留 `768x768` 作为显式方图预设，也可以选择通用横屏、竖屏、方屏规格。面向图片生成模型的高分辨率方图，例如 `1024x1024`、`2048x2048`，属于素材尺寸预设，不属于标准视频输出预设。开启“图片尺寸同步到视频”时，素材尺寸等于最终画布尺寸。
...
- 合同提供向后兼容解析。旧值 `1k`、`2k`、`4k` 在横屏和竖屏方向映射到对应方向下的 HD、Full HD、4K；旧的方屏请求保留原有像素输出，避免历史任务从 `1024x1024` 意外变成 `1080x1080`。
...
- `video_resolution_preset: "1k"` 在横屏/竖屏方向映射到对应方向下的 HD，在方屏方向保持旧输出 `1024x1024`。
- `video_resolution_preset: "2k"` 在横屏/竖屏方向映射到对应方向下的 Full HD，在方屏方向保持旧输出 `2048x2048`。
- `video_resolution_preset: "4k"` 在横屏/竖屏方向映射到对应方向下的 4K，在方屏方向保持旧输出 `4096x4096`。
...
- 显式像素尺寸必须为正整数。
- 如果未来引入高级自定义尺寸，必须单独验证宽高范围、偶数约束、平台兼容性、模板方向和宽高比兼容性。
...
- 标准视频输出预设列表中不存在 1920x720、1024x1024、2048x2048 这类非通用交付规格。
- 旧的 `1k`、`2k`、`4k` 请求仍能生成：横屏/竖屏按通用尺寸归一化，方屏保持历史像素输出。
