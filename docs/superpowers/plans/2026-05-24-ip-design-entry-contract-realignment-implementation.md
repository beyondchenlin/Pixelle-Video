...
For direct media entry points such as `/image/generate`, workbench frame regeneration, and prompt-prefix previews, write the final prompt artifact before the media call. The artifact must include `requested_workflow` and the resolved `workflow` when the caller used a default, plus media type and media/canvas dimensions. Pass a `media_prompt_trace_context` into every media call and make `MediaService` reject calls that lack that context, so `FrameProcessor` or any other caller cannot bypass the final-prompt artifact. Use the same resolved workflow key in the artifact and the media call; workflow trace resolution must fail before media execution if it cannot resolve. Prompt-prefix preview traces are written before media and updated with the resulting preview path or left as a failure artifact if media generation fails. `PromptPlanBundle`, `PromptPlan`, and `ImagePromptDraft` must store source trace ids from the prompt-generation LLM batch so the final prompt can be replayed back to exact LLM request/response payloads. If style resolution fails, abort generation instead of falling back to raw prefix concatenation; a mechanically prefixed prompt is not an acceptable final visual prompt. The same rule applies when a resolved style has no prompt template: use structured style profile fields as semantic visual clauses, not raw prefix text. Legacy `image_config.prompt_prefix` is not an implicit style source.
